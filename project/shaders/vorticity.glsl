...
void main() {
	...
	// Gradient of |omega| via neighbors
	float wr = 0.5 * ((vr.y - vl.y) - (vu.x - vd.x));
	float wl = 0.5 * ((vr.y - vl.y) - (vu.x - vd.x));
	float wu = 0.5 * ((vr.y - vl.y) - (vu.x - vd.x));
	float wd = 0.5 * ((vr.y - vl.y) - (vu.x - vd.x));
