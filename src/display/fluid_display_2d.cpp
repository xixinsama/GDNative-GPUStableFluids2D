...
void FluidDisplay2D::_bind_methods() {
	ClassDB::bind_method(D_METHOD("set_sim_target", "path"), &FluidDisplay2D::set_sim_target);
	ClassDB::bind_method(D_METHOD("get_sim_target"), &FluidDisplay2D::get_sim_target);
...
void FluidDisplay2D::set_sim_target(const NodePath &p_path) { _sim_target = p_path; }
NodePath FluidDisplay2D::get_sim_target() const { return _sim_target; }
...
void FluidDisplay2D::_ready() {
	...
	if (_sim_target.is_empty()) return;

	Node *src = get_node_or_null(_sim_target);
	if (!src) return;
...
void FluidDisplay2D::_process(double p_delta) {
	...
	switch (_display_mode) {
		...
		case DisplayMode::Vorticity:  // Vorticity field not directly available; fall back to velocity
